...
%% Available ESDL Tessalator callbacks
...
-define(ESDL_TESSCB_GLVERTEX,    3).       %% calls gl:vertex3dv(vertex)
-define(ESDL_TESSCB_ERROR_PRINT, 4).       %% Prints error on stderr.
-define(ESDL_TESSCB_COMBINE,     5).       %% Adds vector, and adjusts user data
-define(ESDL_TESSCB_GLEDGEFLAG,  6).       %% calls gl:edgeFlag(Flag)
-define(ESDL_TESSCB_VERTEX_DATA, 7).       %% calls various
...
-define(GLU_NURBS_TEX_COORD_EXT, ?GLU_NURBS_TEXTURE_COORD_EXT).
-define(GLU_NURBS_TEX_COORD_DATA_EXT, ?GLU_NURBS_TEXTURE_COORD_DATA_EXT).
%%%% Filter4TypeSGIS 
